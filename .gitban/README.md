<!--
  ...
  Shipped and refreshed by `gitban update` (reference material, not user state).
...
If you can see this file, `gitban update` already ran. Here's how to start.
...
**Forking a gitban-managed upstream?** Run `gitban update --contributing` once. It
seeds a `.gitban/forge.yaml` collaboration topology (skip-if-exists), validates
it, merges the overlay `.gitignore` entries for you, renders a
`CONTRIBUTING-gitban.md` at your repo root, and prints the `git remote add`
commands you still need to run. gitban **never modifies your git remotes** — it
reports the delta and leaves applying it to you. Re-running is safe.
...
## What `gitban update` put in your repo
...
1. **`CLAUDE.md`** — the project-wide brief every agent reads each session. Put
   ...
   never overwritten by `gitban update`.
...
3. **`.claude/skills/<skill>/SKILL.local.md`** — per-skill, system-specific
   ...
   that skill on every deploy. It's yours: `gitban update` never overwrites it.

Re-run **`gitban update`** after upgrading gitban to refresh the shipped
