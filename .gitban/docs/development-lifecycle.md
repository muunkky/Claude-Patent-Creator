<!--
  Shipped by `gitban update` to `.gitban/docs/development-lifecycle.md`.

  This is the canonical, client-facing description of how development works
  in a gitban-scaffolded repository. It is refreshed on every `gitban update`
...
```
  ...
  └─────────────┘     └──────────────┘     └───────────────┘     └─────────┘
   HITL — optional     HITL — optional      fully autonomous      human-triggered
   (writer↔reviewer)   (architect↔reviewer) (stateless agents)    (pr skill)
```

**The whole pipeline is autonomous — it can run end-to-end with no one watching.**
Humans can step in wherever they want, but nothing requires it. The agents before
the dispatcher (everything in **Plan** and **Decompose**) are the
**HITL (human-in-the-loop) agents** — not a manual step, but the natural place to
intervene *if you choose to*: steer a draft, accept it, or send it back. Leave them
alone and the adversarial writer↔reviewer loop converges on its own. **Execute** and
**Land** run hands-off by design.
